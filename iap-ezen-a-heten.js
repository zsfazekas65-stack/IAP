...
function addMoodleTest(w,anchor){const tests=moodleTests.filter(t=>t.week===w);if(!tests.length||!anchor)return;const style=document.createElement('style');style.textContent='.iap-moodle-alert{max-width:1420px;margin:18px auto 0;padding:0 44px}.iap-moodle-card{display:flex;align-items:center;justify-content:space-between;gap:24px;padding:20px 24px;border:1px solid #f0c36a;border-left:6px solid #ef8b00;border-radius:12px;background:linear-gradient(120deg,#fff9ea,#fff);box-shadow:0 7px 20px rgba(90,60,10,.09)}.iap-moodle-label{display:block;color:#b46200;font-size:11px;font-weight:900;letter-spacing:.12em}.iap-moodle-card h3{margin:5px 0 4px;color:#533100;font-size:22px}.iap-moodle-card p{margin:0;color:#745b36;font-weight:700}.iap-moodle-btn{flex:0 0 auto;padding:13px 20px;border-radius:8px;background:#ef8b00;color:#fff!important;font-weight:900;font-size:13px;letter-spacing:.04em;box-shadow:0 4px 10px rgba(160,90,0,.2)}.iap-moodle-btn:hover{filter:brightness(1.06);transform:translateY(-1px)}@media(max-width:760px){.iap-moodle-alert{padding:0 18px}.iap-moodle-card{align-items:flex-start;flex-direction:column}.iap-moodle-btn{width:100%;text-align:center}}';document.head.appendChild(style);const box=document.createElement('div');box.className='iap-moodle-alert';box.innerHTML=tests.map(t=>`<div class="iap-moodle-card"><div><span class="iap-moodle-label">📝 AKTUÁLIS MOODLE-TESZT • ${esc(t.grade)}</span><h3>${esc(t.subject)} – ${esc(t.title)}</h3><p>${esc(t.detail)} • ${w}. tanítási hét</p></div><a class="iap-moodle-btn" href="${esc(t.url)}" target="_blank" rel="noopener noreferrer">TESZT MEGNYITÁSA →</a></div>`).join('');anchor.parentNode.insertBefore(box,anchor.nextSibling)}
async function init(){addTimetableButton();addKretaButton();addMobileSupport();const grades=document.querySelector('.grades');if(!grades||document.querySelector('.iap-week-now'))return;const w=schoolWeek(),sec=document.createElement('section');sec.className='iap-week-now';sec.innerHTML=`<div class="iap-week-head"><div><span class="iap-week-kicker">IAP // AKTUÁLIS TANÍTÁSI HÉT</span><h2>Ezen a héten</h2><p>A tantárgyak aktuális témái egy helyen.</p></div><div class="iap-week-badge"><strong>${w}.</strong><span>HÉT</span></div></div><div class="iap-week-grid">${subjects.map((s,i)=>{const hash=s[3]?`#het${w}`:`#het-${w}`;return `<a class="iap-week-item" href="${s[2]}${hash}"><span class="iap-week-grade">${esc(s[0])}</span><strong>${esc(s[1])}</strong><small id="iap-topic-${i}">Téma betöltése…</small><span class="iap-week-go">MEGNYITÁS →</span></a>`}).join('')}</div>`;grades.parentNode.insertBefore(sec,grades);addMoodleTest(w,sec);subjects.forEach(async(s,i)=>{const e=document.getElementById(`iap-topic-${i}`);if(e)e.textContent=await topic(s[2],w)})}
