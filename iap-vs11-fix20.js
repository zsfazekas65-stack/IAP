/* IAP – Villamos szerelések 11 – biztos gombjavítás a 20., 26. és 27. héthez */
(()=>{
'use strict';
...
const JAVITASOK={
  ...
  27:{
    cim:'27. hét – Hurokimpedancia mérése és a védelem ellenőrzése',
    leiras:'79–81. óra: hibahurok és hurokimpedancia fogalma, várható hibaáram, védelmi készülékek működési feltételei, mérési pontok, dokumentálás és hibakeresés.',
    fajl:'27'
  }
};
...
function javitHet(szam,adat){
  ...
  hrefek(adat.fajl).forEach((href,i)=>{
    ...
    else { a.removeAttribute('download'); a.setAttribute('target','_blank'); a.setAttribute('rel','noopener'); }
  });
...
const KIHIVASOK={
  26:{
    kerdes:'Szigetelési ellenállás mérése előtt mi a helyes első lépés?',
    jo:'A vizsgált áramkört feszültségmentesítem, leválasztom az érzékeny készülékeket, majd ellenőrzöm a feszültségmentes állapotot.',
    rossz:['Bekapcsolt állapotban azonnal 500 V mérőfeszültséget kapcsolok rá.','A PE vezetőt megszakítom, hogy ne befolyásolja a mérést.','Csak a kismegszakítót kapcsolom le, további ellenőrzés nélkül.'],
    ok:'Helyes. A szigetelési ellenállás mérése csak biztonságosan előkészített, feszültségmentes áramkörön végezhető.',
    bad:'Nem ez a helyes sorrend. Először feszültségmentesítés, leválasztás és ellenőrzés szükséges.'
  },
  27:{
    kerdes:'Egy dugaszolóaljzat működik, de a hurokimpedancia-mérés gyanúsan nagy értéket ad. Elfogadható-e csak azért, mert a fogyasztó működik?',
    jo:'Nem. A működés nem bizonyítja a hibavédelem megfelelőségét; a PE-utat, a kötéseket és a védelmi készülék feltételeit ellenőrizni kell.',
    rossz:['Igen, mert ami működik, az villamosan biztosan rendben van.','Igen, ha a kismegszakító nincs leoldva.','Igen, ha a burkolat sértetlen és nem látszik hiba.'],
    ok:'Helyes. A működő fogyasztó nem igazolja automatikusan a hibavédelmet.',
    bad:'Nem jó döntés. A hurokimpedancia a hiba esetén várható lekapcsolási feltétel megítéléséhez fontos.'
  }
};
...
function erosites(){
  javit();
  [100,300,700,1500,3000,5000].forEach(ms=>setTimeout(javit,ms));
}
