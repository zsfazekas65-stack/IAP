...
(function () {
  ...
  function javitas11HetGombok(){
    const oldal=location.pathname.split('/').pop().toLowerCase();
    const map={
      'villamosszereles11.html':'villamosszereles11',
      'elektrotechnika11.html':'elektrotechnika11'
    };
    const mappa=map[oldal];
    if(!mappa)return;
    const h=[...document.querySelectorAll('.het')].find(x=>parseInt((x.querySelector('.het-szam')?.textContent||'').trim(),10)===11);
    if(!h)return;
    const g=[...h.querySelectorAll('.gombok a')];
    if(g.length<4)return;
    const base='https://zsfazekas65-stack.github.io/IAP/';
    const hrefek=[
      `${base}tananyagok/${mappa}/11_het.pdf?v=20260906b`,
      `${base}ppt/${mappa}/11_het.pptx?v=20260906b`,
      `${base}feladatok/${mappa}/11_het_feladat.pdf?v=20260906b`,
      `${base}gyakorlatok/${mappa}/11_het_gyakorlat.pdf?v=20260906b`
    ];
    g.slice(0,4).forEach((a,i)=>{
      ...
      else {a.setAttribute('target','_blank');a.removeAttribute('download');}
    });
  }
  ...
  function indul(){subjectDesignBetoltese();javitas11HetGombok();kiemelAktualisHet();kihivasMotorBetoltese();haladasKovetese()}
