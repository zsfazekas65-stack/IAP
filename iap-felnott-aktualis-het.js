...
function loadChallenge(){if(document.querySelector('script[src*="iap-kihivas-loader.js"]'))return;const s=document.createElement('script');s.src='iap-kihivas-loader.js?v=2';s.async=false;document.body.appendChild(s)}
function init(){style();const w=schoolWeek();document.querySelectorAll('.het').forEach(h=>h.classList.remove('aktualis'));document.querySelectorAll('.iap-aktualis-vissza').forEach(x=>x.remove());const target=document.getElementById('het'+w)||document.getElementById('het-'+w)||[...document.querySelectorAll('.het')].find(h=>parseInt(h.querySelector('.het-szam')?.textContent||'',10)===w);if(target){target.classList.add('aktualis');if(!target.id)target.id='het'+w;const a=document.createElement('a');a.className='iap-aktualis-vissza';a.href='#'+target.id;a.innerHTML='↘ Aktuális hét <strong>'+w+'.</strong>';document.body.appendChild(a)}loadChallenge()}
